...
class EmbeddingCallRecord:
    """model_calls에 남길 청크 embedding 호출 결과."""
...
async def prepare_chunk_embeddings(
...
) -> Tuple[List[List[float]], Optional[EmbeddingCallRecord]]:
    """Chunk 순서에 맞춘 embedding 목록을 만든다.

    이전 판과 embedding 입력이 같은 Chunk는 저장된 vector를 복사하고
    나머지만 한 번의 요청으로 생성한다. 재사용만으로 채워지면 호출이 없으므로
    기록할 model call도 없다.
    """
    ...
    if not missing_positions:
        return [list(reusable[input_hash]) for input_hash in input_hashes], None
    ...
    record = EmbeddingCallRecord(
        generated_count=len(missing_positions),
        reused_count=len(chunks) - len(missing_positions),
