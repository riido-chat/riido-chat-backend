...
class GenerationContextTest(unittest.TestCase):
    ...
    def test_source_planning_prompt_preserves_scope_and_all_evidence(self) -> None:
        ...
        self.assertIn("`예를 들어`로 여러 상황", SOURCE_PLANNING_PROMPT_V11)
        self.assertIn("별도 정보 단위로 과분해하지", SOURCE_PLANNING_PROMPT_V11)
        self.assertIn("가정형으로 묻는 것", SOURCE_PLANNING_PROMPT_V11)
        self.assertIn("실제 상태나 날짜", SOURCE_PLANNING_PROMPT_V11)
    ...
    def test_answer_prompt_avoids_partial_or_duplicate_answers(self) -> None:
        ...
        self.assertIn("임의로 WITHHELD로 바꾸지", ANSWER_PROMPT_V17)
        self.assertIn("계획에 따라 답하세요", ANSWER_PROMPT_V17)
